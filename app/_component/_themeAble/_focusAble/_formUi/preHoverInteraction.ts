...
export const renderedSym = Symbol("rendered")


const maxPxPerFrame = 1.4 // basically speed
const overShoot = 22 // how much the background moves
const targetOverflow = 20 // how much the target is bigger than the element, meaning how close you have to get to the edge to trigger the effect
const targetOverflowWidthStr = `calc(100% + ${targetOverflow * 2}px)`
const activeTargetOverflow = 45
const activeTargetOverflowWidthStr = `calc(100% + ${activeTargetOverflow * 2}px)`
const qlance = .4 // factor by which the hitbox gets larger once the cursor is inside the target
const factorByWhichToRestrainContent = .65 // percentage of movement of content as opposed to the background. 1 = same movement
...
export default function(root: HTMLElement, target: HTMLElement, moveElement: HTMLElement,  evTarget: HTMLElement, contentBody?: Element) {
  ...
  const followRuntimeFunc = (delta: number) => {
    ...
    if (contentBody) contentBody.css({translateX: renderedX * factorByWhichToRestrainContent, translateY: renderedY * factorByWhichToRestrainContent})
  }
