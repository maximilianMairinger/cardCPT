import animationFrame from "animation-frame-delta"
import delay from "tiny-delay";
import { ElementList, EventListener } from "extended-dom";
import { Data, DataBase } from "josm";
import Button from "../_button/button";
import FocusAble from "../focusAble"
import { Theme } from "../../themeAble"
import { renderedSym } from "./preHoverInteraction"

if (window.TouchEvent === undefined) window.TouchEvent = class SurelyNotTouchEvent {} as any


// todo: glow effect like on the google chrome tabs




export type ReadonlyData<T> = Omit<Data<T>, "set">

// distance between two points
function distance(p1: [number, number], p2: [number, number]) {
    return Math.sqrt(Math.abs(Math.pow(p1[0] - p2[0], 2) + Math.pow(p1[1] - p2[1], 2)));
}

let cursor: HTMLElement
const cursorSize = 8
const cursorColor = "black"

let snatchCursorTo: HTMLElement
// This is a global setting. When enabled a porint will follow the cursor. When a formUi element is hovered, the point will morph to the shape of the element. The point element exsists on the body once.
export function morphCursorToHover() {
  cursor = ce("follow-cursor")

  document.body.apd(cursor)
  cursor.css({
    position: "absolute",
    display: "block",
    pointerEvents: "none",
    top: -cursorSize/2,
    left: -cursorSize/2,
    width: cursorSize,
    height: cursorSize,
    borderRadius: 100,
    backgroundColor: cursorColor,
    zIndex: 50,
  })

  // follow the mouse

  let mouseX = 0
  let mouseY = 0
  document.on("mousemove", (e) => {
    mouseX = e.clientX
    mouseY = e.clientY
  })

  let cursorX = 0
  let cursorY = 0

  document.on("mouseenter", (e) => {
    cursorX = e.clientX
    cursorY = e.clientY
    cursor.show()
  })

  document.on("mouseleave", (e) => {
    cursor.hide()
  })

  

  let curWidth = cursorSize
  let curHeight = cursorSize

  const followRuntimeFunc = (delta: number) => {

    if (snatchCursorTo) {
      const snatchCursorToRect = snatchCursorTo.getBoundingClientRect()
      if (snatchCursorToRect === undefined) return
      let mouseX = snatchCursorToRect.x
      let mouseY = snatchCursorToRect.y

      const totalDiffX = mouseX - cursorX
      const totalDiffY = mouseY - cursorY


      if (Math.abs(totalDiffX) < 1 && Math.abs(totalDiffY) < 1) {
        cursorX = mouseX
        cursorY = mouseY
        cursor.css({translateX: cursorX, translateY: cursorY})
        return
      }
      else {
        const xDiffThisFrame = totalDiffX * delta / 7
        const yDiffThisFrame = totalDiffY * delta / 7

        const widthDiff = snatchCursorToRect.width - curWidth
        const heightDiff = snatchCursorToRect.height - curHeight

        const thisFrameWidthDelta = widthDiff * delta / 7
        const thisFrameHeightDelta = heightDiff * delta / 7


        curWidth += thisFrameWidthDelta
        curHeight += thisFrameHeightDelta


        // factor out the x distance
        cursorX += xDiffThisFrame 
        cursorY += yDiffThisFrame

        

        
        cursor.css({translateX: cursorX, translateY: cursorY, width: curWidth, height: curHeight})
        return
      }
    }
    else {
      const widthDiff = cursorSize - curWidth
      const heightDiff = cursorSize - curHeight

      const thisFrameWidthDelta = widthDiff * delta / 7 + (Math.abs(widthDiff) > 2 ? Math.sign(widthDiff) * 2 : 0)
      const thisFrameHeightDelta = heightDiff * delta / 7 + (Math.abs(heightDiff) > 2 ? Math.sign(heightDiff) * 2 : 0)

      curWidth += thisFrameWidthDelta
      curHeight += thisFrameHeightDelta

      const totalDiffX = mouseX - cursorX
      const totalDiffY = mouseY - cursorY

      const xDiffThisFrame = totalDiffX * delta / 4
      const yDiffThisFrame = totalDiffY * delta / 4

      // factor out the x distance
      cursorX += xDiffThisFrame 
      cursorY += yDiffThisFrame

      
      cursor.css({translateX: cursorX, translateY: cursorY, width: curWidth, height: curHeight})
    }

    
  }

  animationFrame(followRuntimeFunc)
}


morphCursorToHover()


export default abstract class FormUi<T extends false | HTMLElement | HTMLAnchorElement = false | HTMLElement> extends FocusAble<T> {
  private rippleElements: HTMLElement;
  private waveElement: HTMLElement;
  public validMouseButtons = new Set([0])

  public moveBackground = this.q("move-me") as HTMLElement;
  public get cursorMorphBackgroundColor() {
    return this.body.dummy.css("backgroundColor")
  }

  public userFeedbackMode: DataBase<{
    ripple: boolean | "late",
    hover: boolean,
    focus: boolean | "direct",
    active: boolean,
    enabled: boolean
  }>


  private coverElems = {
    hover: this.q("hover-cover"),
    click: this.q("click-cover"),
  }


  public isFocused: ReadonlyData<boolean>
  public enabled: Data<boolean>
  constructor(componentBodyExtension?: HTMLElement | false) {
    super(componentBodyExtension)
    if (this.componentBody instanceof HTMLElement) this.componentBody.id = "componentBody"

    this.userFeedbackMode({
      ripple: true,
      hover: true,
      active: false,
      enabled: true
    })

    this.addClass("rippleSettled")
    this.moveBackground.apd(this.focusManElem)

    this.enabled = new Data(true) as Data<boolean>
    this.enabled.get((enabled) => {
      if (enabled) {
        if (this.preHoverAnimations) this.preHoverAnimations.enable()
      }
      else {
        if (this.preHoverAnimations) this.preHoverAnimations.disable()
      }

    }, false)

    const enabledUserFeedbackSub = this.enabled.get((enabled) => {
      if (enabled) this.removeClass("disabled")
      else this.addClass("disabled")
    })

    this.userFeedbackMode.enabled.get((feedback) => {
      if (feedback) enabledUserFeedbackSub.activate(true)
      else {
        this.removeClass("disabled")
        enabledUserFeedbackSub.deactivate()
      }
    }, false);


    (() => {
      let lastDelay = delay(0)
      if (cursor) {
        const overListener = new EventListener(this.componentBody as HTMLElement, "mouseenter", () => {
          cursor.anim({
            borderRadius: this.css("borderRadius"),
            top: 0,
            left: 0,
          }, 300)
          lastDelay.cancel()
          cursor.anim({background: this.cursorMorphBackgroundColor}, {duration: 160, easing: "easeOut"})
          

          snatchCursorTo = this.moveBackground
        })

        overListener.deactivate()
  
        const outListener = new EventListener(this.componentBody as HTMLElement, "mouseleave", () => {

          cursor.anim({
            borderRadius: 100,
            top: -cursorSize/2,
            left: -cursorSize/2,
          }, {duration: 300, easing: "easeOut"})
          lastDelay.cancel()
          lastDelay = delay(150, () => {
            cursor.anim({
              background: cursorColor
            }, {duration: 150, easing: "easeOut"})
          })


          snatchCursorTo = undefined
        })
        outListener.deactivate()
    
    
        this.userFeedbackMode.hover.get((y) => {
          if (y) {
            overListener.activate()
            outListener.activate()
          }
          else {
            overListener.deactivate()
            outListener.deactivate()
          }
        })
      }
      else {
        this.userFeedbackMode.hover.get((y) => {
          this.coverElems.hover[y ? "addClass" : "removeClass"]("active")
        })
      }
    })()
    

    this.on("focus", (e) => {
      if (snatchCursorTo !== undefined && snatchCursorTo !== this.moveBackground) {
        snatchCursorTo = this.moveBackground
        this.addClass("clickFocus")
      }
    })
    

    this.userFeedbackMode.active.get((y) => {
      this.coverElems.click[y ? "addClass" : "removeClass"]("active")
    })

    const preLs = (() => {

      const preLs = [] as EventListener[]
      preLs.add(this.on("mousedown", (e) => {
        if (!touched) {
          if (this.validMouseButtons.has(e.button)) {
            this.initRipple(e);
          }
        }
      }, {capture: true}))

      let touched = false
      preLs.add(this.on("touchend", () => {
        touched = true
        delay(100).then(() => {
          touched = false
        })
      }, {capture: true}))

      preLs.add(this.on("touchstart", (e) => {
        this.initRipple(e);
      }, {capture: true}))



      return preLs
    })();



    const curLs = (() => {

      const curLs = [] as EventListener[]
      const e = this.on("mousedown", (e) => {
        if (this.validMouseButtons.has(e.button)) this.initRipple(e);
      }, {capture: true})
      e.deactivate()
      curLs.add(e as any)

      return curLs
    })();


    this.rippleElements = ce("button-waves");
    this.moveBackground.apd(this.rippleElements);


    this.userFeedbackMode.ripple.get((mode) => {
      if (mode) {
        this.rippleElements.css({zIndex: "initial"})
        if (mode !== "late") {
          for (let p of preLs) {
            p.activate()
          }
          for (let c of curLs) {
            c.deactivate()
          }
        }
        else {
          for (let c of curLs) {
            c.activate()
          }
          for (let p of preLs) {
            p.deactivate()
          }
        }
      }
      else {
        this.rippleElements.css({zIndex: -1})
        for (let c of curLs) {
          c.deactivate()
        }
        for (let p of preLs) {
          p.deactivate()
        }
      }
    }, true)

    
    const isFocused = (this as any).isFocused = new Data(false)
    this.on("focusin", () => {(this as any).isFocused.set(true)})
    this.on("focusout", () => {(this as any).isFocused.set(false)})





    const hovPreDet = ce("prehover-detector")
    hovPreDet.on("mousedown", (e) => {
      if (isFocused.get()) {
        e.preventDefault()
        e.stopPropagation()
      }
    }, true)
    this.componentBody.prepend(hovPreDet);
    const root = ce("root-bounds")
    this.apd(root);



    const contentBody = this.getContentBody() as Element
    if (contentBody) setTimeout(async () => {
      (await contentBody).css({zIndex: 60, position: "relative", display: "block"})
    })


    if (window.matchMedia && window.matchMedia("(hover:hover)").matches) {
      import("./preHoverInteraction").then(async ({default: f}) => {
        this.preHoverAnimations = f(root as any, hovPreDet, this.moveBackground as any, this.componentBody as any, await this.getContentBody() as Element)
      })
    }

    





    this.waveElement = ce("button-wave-container").apd(ce("button-wave"))



    
  }
  protected abstract getContentBody(): Element | void | Promise<Element>
  private preHoverAnimations: {disable: () => void, enable: () => void}


  protected fadeRipple: ((anim?: boolean) => void)[] = []
  protected rippleElems: ElementList<Element & {fade?: ((animation?: boolean) => Promise<void>) & {auto?: boolean}}> = new ElementList
  protected initRippleCb = (e?: MouseEvent | TouchEvent | KeyboardEvent | "center") => () => {}


  public rippleSettled = Promise.resolve()
  public initRipple(e?: MouseEvent | TouchEvent | KeyboardEvent | "center"): () => void {
    if (!this.enabled.get()) return
    let rippleSettled: Function
    const myRippleSettledProm = this.rippleSettled = new Promise((res) => {rippleSettled = res})
    this.removeClass("rippleSettled")
    const fadeRippleCb = this.initRippleCb()

    let rippleWaveElemContainer = this.waveElement.cloneNode(true) as Element;
    let rippleWaveElem = rippleWaveElemContainer.children[0]
    this.rippleElements.apd(rippleWaveElemContainer); 

    const fadeAnimIfPossible: Function & {auto?: boolean} = () => {
      setTimeout(() => {
        if (!fadeAnim.auto) return
        this.rippleElems.rmV(rippleWaveElem)
        return fadeAnim()
      })
    }
    
    

    const fadeAnim = async (anim = true) => {
      fadeRippleCb()

      if (anim) {
        try {
          await rippleWaveElem.anim({opacity: 0}, 500);  
        } catch (error) {
          
        }
        await delay(500)
      }
      
      rippleWaveElemContainer.remove()
    }
    fadeAnim.auto = true;

    (rippleWaveElem as any).fade = fadeAnim
    this.rippleElems.add(rippleWaveElem)

    this.fadeRipple.add(fadeAnim)

    let fadeisok = () => {
      if (rdyToFade) fadeAnimIfPossible();
      else rdyToFade = true;
    }

    let once = true
    const uiOut = (e) => {
      if (once) {
        once = false;
        fadeisok()
      }
    }

    let x: number;
    let y: number;



    const body = {
      width: this.width(),
      height: this.height()
    }

    const ripple = {
      diameter: 25,
      radius: 25 / 2,
    }

    if (e instanceof MouseEvent || e instanceof TouchEvent) {
      if (e instanceof TouchEvent) {
        //@ts-ignore
        e.pageX = e.touches[e.touches.length-1].pageX
        //@ts-ignore
        e.pageY = e.touches[e.touches.length-1].pageY


        document.body.on("touchcancel", uiOut, {once: true});
        document.body.on("touchend", uiOut, {once: true});
        this.on("blur", uiOut, {once: true});
      }
      else {
        document.body.on("mouseup", uiOut, {once: true});

      }
      let offset = this.absoluteOffset();
      x = (e as MouseEvent).pageX - offset.left;
      y = (e as MouseEvent).pageY - offset.top;

      
    }
    else {
      x = body.width / 2;
      y = body.height / 2;

      if (e instanceof KeyboardEvent) {
        this.on("keyup", uiOut, {once: true});
        this.on("blur", uiOut, {once: true});
      }
    }
    rippleWaveElem.css({
        marginTop: y - ripple.radius,
        marginLeft: x - ripple.radius
    });
    let rdyToFade = false;

    
    
    const cursorPoint = [x, y] as [number, number]
    let maxDistance = Math.max(
      distance(cursorPoint, [0, 0]), 
      distance(cursorPoint, [body.width, body.height]), 
      distance(cursorPoint, [body.width, 0]), 
      distance(cursorPoint, [0, body.height])
    )

    const scale = maxDistance / ripple.radius

  
    
    
    const animProm = rippleWaveElem.anim([{transform: "scale(0)", offset: 0}, {transform: "scale(" + scale + ")"}], {duration: 300 + maxDistance, easing: "linear"}).then(fadeisok);

    animProm.then(() => {
      if (this.rippleSettled === myRippleSettledProm) this.addClass("rippleSettled")
      rippleSettled()
    })
    
    

    return fadeisok
  }


  public pug(): string {
    return super.pug() + require("./formUi.pug").default
  }
  stl() {
    return super.stl() + require("./formUi.css").toString()
  }
  
}
